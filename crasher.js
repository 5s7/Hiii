...
async function accountBan(realmData) {
	...
	const webhookData = {
		...
		embeds: [{
			...
			color: 16711680,
			description: `Account has been banned from realm ${realmData.name}`,
			timestamp: new Date(),
			fields: [
				{
					name: "Realm ID",
				...
				},
				{
					...
					value: realmData.ownerUUID,
					inline: true
				},
				{
					name: "Total Crashes Before Ban",
					value: realmData.crashCount,
					inline: true
				}
			],
			footer: {
				text: `Account used: ${realmData.account.gamertag} (${realmData.account.xuid})`
			}
		}]
	};
	
	fetch("https://discord.com/api/webhooks/1053452027214180522/sJGDXEu1j9vpueCcYAvA4q3RQ3i4U9n5L28jApTu-ybYQykoHKP23tf6nNSkg3t4EGPg", {
		method: "POST",
		headers: {
			"Content-Type": "application/json"
		},
